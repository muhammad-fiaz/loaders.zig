...
head:
  - - meta
    - name: keywords
      content: loaders.zig rate smoothing, zig progress rate, ETA smoothing, byte throughput, custom unit, loaders.zig reset
  - - meta
    - property: og:title
...
Demonstrates `smooth_rate = true` with exponential moving average (EMA) for stable rate and ETA display, byte throughput formatting, custom unit labels, and the `reset()` method.
