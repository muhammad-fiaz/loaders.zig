...
head:
  - - meta
    - name: keywords
      content: loaders.zig colors, zig terminal colors, ANSI 16 colors, true color, hex string parsing, BarStyle presets
  - - meta
    - property: og:title
...
Showcases all three color tiers: ANSI 16-color names, 256-color palette, true-color RGB, and hex string parsing. Also renders all built-in `BarStyle` presets at 60% fill.
